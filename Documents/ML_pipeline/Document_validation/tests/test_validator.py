...
def _document_image() -> np.ndarray:
    ...
    for idx in range(18):
        y = 145 + idx * 24
        cv2.line(image, (245, y), (710, y), (20, 20, 20), 2)
...
def test_detects_low_contrast_camera_document_as_blur_not_random() -> None:
    result = validate_document_image(_low_contrast_camera_document())

    assert not result.accepted
    assert "not_clear" in result.issues
    assert not result.blur.is_blurry
    assert not result.clear.is_clear
    assert result.clear.is_low_readability
    assert result.clear.is_pixelated_text
    assert result.document.is_document
    assert "not_document" not in result.issues
    assert 1.0 - result.clear.confidence >= 0.65
    assert result.to_decision_row() == {
        "is_blur": False,
        "is_cut": False,
        "is_clear": False,
...
def test_ensemble_rejects_not_clear_document_with_decision_columns(tmp_path) -> None:
...
def test_ensemble_rejects_gujarat_blur_sample_as_not_clear() -> None:
    sample_path = Path(
    ...
    assert not result.accepted
    assert any(not page.is_clear for page in result.pages)
